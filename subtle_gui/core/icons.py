    ...
    def icon(self, key: str) -> QIcon:
        ...
        if key not in self._cache:
            self._cache[key] = QIcon(
                _IconEngine(BASE.replace(b"{content}", RAW_SVG.get(key, b"")).replace(b"{foreground}", self._color))
            )
...
class _IconEngine(QIconEngine):
    ...
    def paint(self, painter: QPainter | None, rect: QRect, mode: QIcon.Mode, state: QIcon.State) -> None:
        """SVG icon painter."""
